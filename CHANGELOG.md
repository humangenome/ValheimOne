...
- Verified against Valheim 1.0.15 (dedicated-server build 25390671); `SupportedGameVersion` is `1.0.15`, so the startup banner no longer warns on the current game. All 484 static references and 51 reflection targets resolve unchanged, and the Harmony patch contract is identical to the 1.0.12 baseline.
